...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = 1/(2*len(real_images)) * torch.sum((D(real_images)-1)**2)
            
            # 2. Sample noise
            noise = sample_noise(len(real_images), 100)

            # 3. Generate fake images from the noise
            fake_images = G(noise)

            # 4. Compute the discriminator loss on the fake images
            D_fake_loss = 1/(2*len(real_images)) * torch.sum((D(fake_images))**2)
            ...
            G_loss = 1/(len(real_images)) * torch.sum((D(fake_images)-1)**2)
            ...
            if iteration % opts.log_step == 0:
                print('Iteration [{:4d}/{:4d}] | D_real_loss: {:6.4f} | D_fake_loss: {:6.4f} | G_loss: {:6.4f}'.format(
                       iteration, total_train_iters, D_real_loss[0], D_fake_loss[0], G_loss[0]))
