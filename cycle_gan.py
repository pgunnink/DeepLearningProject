...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = 1/len(images_X)*torch.sum((D_X(images_X)-1)**2)
        D_Y_loss = 1/len(images_Y)*torch.sum((D_Y(images_Y)-1)**2)
        ...
        D_X_loss = 1/(len(images_Y))*torch.sum(D_X(fake_X)**2)

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = 1/(len(images_X))*torch.sum(D_Y(fake_Y)**2)
        ...
        g_loss = 1/(len(images_Y)) * torch.sum((D_X(fake_X)-1)**2)
        ...
        g_loss = 1/len(images_X) * torch.sum((D_Y(fake_Y)-1)**2)
...
if __name__ == '__main__':
    ...
    if opts.weight_norm:
        opts.sample_dir = 'samples_cyclegan_cycle_weight_norm'
        weight_norm = opts.weight_norm

    if opts.instance_norm:
        opt.sample_dir = 'samples_cyclegan_cycle_instance_norm'
        instance_norm = opts.instance_norm
